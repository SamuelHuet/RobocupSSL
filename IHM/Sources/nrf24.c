...
unsigned char nrf24_payloadLength(void)
{
	...
	dummy=spi_transfer(R_RX_PL_WID);
...
void nrf24_getData(unsigned char* data) 
{
	...
	dummy=spi_transfer( R_RX_PAYLOAD );
...
{    
	...
	dummy=spi_transfer(W_TX_PAYLOAD);
...
unsigned char spi_transfer(unsigned char tx)
{
	...
	if(SPIS_SPTEF){  //Tant que le buffer n'est pas vide
		SPID=tx;
	}
...
{
	nrf24_csn_digitalWrite(LOW);
	dummy=spi_transfer(W_REGISTER | (REGISTER_MASK & reg));
	dummy=spi_transfer(value);
	nrf24_csn_digitalWrite(HIGH);
}
...
{
	nrf24_csn_digitalWrite(LOW);
	dummy=spi_transfer(R_REGISTER | (REGISTER_MASK & reg));
